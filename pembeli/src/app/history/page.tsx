          ...
          orders.map((order, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-3 border-b border-border-subtle pb-3">
                <p className="text-xs font-bold text-text-muted">{formatDate(order.created_at || order.date)}</p>
                <div className={`px-3 py-1 rounded-full border text-[10px] font-bold ${getStatusColor(order.status)}`}>
                  {order.status}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div>
                  <h3 className="font-bold text-sm text-text-primary">{order.store_name || order.storeName}</h3>
                ...
                <div className="flex justify-between items-end mt-2 pt-2 border-t border-border-subtle border-dashed">
                  <div className="text-left">
                    <p className="text-[10px] text-text-muted font-bold">Total Pembayaran</p>
                    <p className="font-bold text-primary text-sm mt-0.5">Rp. {(order.total_amount || order.totalAmount || 0).toLocaleString('id-ID')}</p>
                  </div>
